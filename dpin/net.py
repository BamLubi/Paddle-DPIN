...
class DPINLayer(nn.Layer):
    def __init__(self, K, emb_dim, max_item, max_context, d_model, h, is_sparse):
        ...
        self.base_module = nn.Sequential(
            nn.Linear(in_features=2*self.emb_dim, out_features=1024, weight_attr=nn.initializer.KaimingUniform()),
            nn.ReLU(),
            nn.Linear(in_features=1024, out_features=512, weight_attr=nn.initializer.KaimingUniform()),
            nn.ReLU(),
            nn.Linear(in_features=512, out_features=128, weight_attr=nn.initializer.KaimingUniform()),
            nn.ReLU()
        )
        ...
        self.interest_agg = InterestAggregation(K, emb_dim, max_item, max_context, d_model, h, is_sparse)
        # Position-wise Non-linear Interaction
        self.non_linear_interaction = nn.Sequential(
            nn.Linear(in_features=3*self.emb_dim, out_features=64, weight_attr=nn.initializer.KaimingUniform()),
            nn.ReLU()
        )
        # Transformer Block
        ...
        self.combination = nn.Sequential(
            nn.Linear(in_features=128+self.d_model+self.emb_dim, out_features=128, weight_attr=nn.initializer.KaimingUniform()),
            nn.ReLU(),
    ...
    def forward(self, hist_item, hist_cat, target_item, target_cat, position):
        ...
        context_feat_emb_val = self.context_feat_emb(target_cat)    # [*, emb_dim]
        item_feat_emb_val = self.item_feat_emb(target_item)         # [*, emb_dim]
        base_module_input = paddle.concat([context_feat_emb_val, item_feat_emb_val], axis=1) # [*, 2*emb_dim]
        ...
        input_non_linear_inter = paddle.concat([position_emb_val, interest_agg], axis=2)
        non_linear_inter_val = self.non_linear_interaction(input_non_linear_inter)
        
        # Transformer Block
        transformer_output = self.transformer(non_linear_inter_val) # [batchsize, 25, 64]
        
        # Position-wise Combination Module
        item_output_unqueeze = paddle.unsqueeze(item_output, axis=1)
        item_output_unqueeze = paddle.tile(item_output_unqueeze, repeat_times=[1,self.K,1])
        com_position_emb_val = self.position_emb_2(position)
        item_pos = paddle.concat([item_output_unqueeze, transformer_output, com_position_emb_val], axis=2)
...
class Transformer(nn.Layer):
    def __init__(self, d_model, h, K):
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(in_features=d_model, out_features=d_model, weight_attr=nn.initializer.KaimingUniform()),
            nn.ReLU(),
            nn.Linear(in_features=d_model, out_features=d_model, weight_attr=nn.initializer.KaimingUniform()),
        )
    ...
    def forward(self, non_linear_inter_val):
        # MultiHeadAttention
        multi_head_output = self.multi_head_attn(non_linear_inter_val, non_linear_inter_val, non_linear_inter_val)
        # Add & Norm
...
class InterestAggregation(nn.Layer):
    def __init__(self, K, emb_dim, max_item, max_context, d_model, h, is_sparse):
        ...
        self.MLP = nn.Sequential(
            nn.Linear(in_features=2*self.emb_dim, out_features=2*self.emb_dim, weight_attr=nn.initializer.KaimingUniform()),
            nn.ReLU(),
            nn.Linear(in_features=2*self.emb_dim, out_features=2*self.emb_dim, weight_attr=nn.initializer.KaimingUniform())
        )
    
    def forward(self, hist_item, hist_cat):
        user_bx_item_val = self.user_bx_item_emb(hist_item)         # [*, K, L] => [*, K, L, E]
        user_bx_context_val = self.user_bx_context_emb(hist_cat)    # [*, K, L] => [*, K, L, E]
        user_bx = paddle.concat([user_bx_item_val, user_bx_context_val], axis=3) # [*, K, L, 2E]
        ...
        output = paddle.sum(user_bx * user_bx_exp, axis=2) / user_bx_exp_sum # [*, K, 2E]
